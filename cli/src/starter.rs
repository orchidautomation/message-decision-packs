...
pub(crate) fn starter_manifest(name: &str, slug: &str, _template: &str) -> Manifest {
    ...
    Manifest {
        ...
        cards: vec![
            ...
            card_ref("channel-policies", "cards/channel-policies.yaml", CardKind::ChannelPolicies, "Channel-specific policy for LinkedIn, email, call prep, and agent briefs.", &["GTM Engineering", "PMM"], &["channel", "linkedin", "email", "call", "prep", "agent", "brief"]),
...
pub(crate) fn starter_cards(_template: &str) -> Vec<(&'static str, Card)> {
    vec![
        ...
        ("fit-rules.yaml", card("fit-rules", CardKind::FitRules, "Fit rules", "ICP, qualification, disqualification, and no-message rules.", &["GTM Engineering", "PMM", "PM"], &["fit", "icp", "disqualifier", "no-message"], vec![
            entry_with_evidence("good-fit-agent-gtm", "Good fit: agent-assisted GTM", "Use when the account is building GTM workflows with agents, Clay/Deepline-style enrichment rows, Codex/Claude Code/OpenCode, or multiple systems that need consistent message context.", &["GTM Engineering", "PMM"], &["README.md", "examples/clay-row.json"]),
            ...
            Entry { id: "bad-fit-sending-only".to_string(), title: "Bad fit: sending-only ask".to_string(), body: "If the request is only to blast, sequence, or auto-send messages without decision context, treat it as out of scope for MDP.".to_string(), applies_to: vec!["GTM Engineering".to_string(), "PMM".to_string()], evidence: vec!["README.md".to_string()], avoid: vec!["blast".to_string(), "auto-send".to_string(), "sequence everyone".to_string()] },
        ])),
        ("signals.yaml", card("signals", CardKind::Signals, "Signals and triggers", "How to interpret enriched rows, LinkedIn context, source material, and account signals.", &["GTM Engineering", "PMM", "PM"], &["signal", "trigger", "source", "clay", "deepline", "linkedin"], vec![
            entry_with_evidence("enriched-row-signal", "Enriched row signal", "Treat Clay, Deepline, CSV, or user-provided enrichment as input evidence. Preserve source and confidence when present, and state weak signals as hypotheses.", &["GTM Engineering", "PMM"], &["examples/clay-row.json"]),
        ...
        ("motions.yaml", card("motions", CardKind::Motions, "Approved motions", "GTM workflows this pack can support as context.", &["GTM Engineering", "PMM"], &["motion", "workflow"], vec![
            ...
            entry_with_evidence("clay-row-to-brief", "Enriched row to brief", "Convert a Clay/Deepline-style row into a message brief before drafting. Keep source fields as inputs, not as proof of claims.", &["GTM Engineering", "PMM"], &["README.md", "examples/clay-row.json"]),
        ])),
        ("channel-policies.yaml", card("channel-policies", CardKind::ChannelPolicies, "Channel policies", "Channel-specific rules for how to use the routed message decisions.", &["GTM Engineering", "PMM"], &["channel", "linkedin", "email", "brief"], vec![
            entry_with_evidence("linkedin-opener", "LinkedIn opener", "Keep the opener short, use one sourced or explicitly hypothetical trigger, one relevant angle, and one low-friction ask.", &["PMM"], &["README.md"]),
            entry_with_evidence("email-follow-up", "Email follow-up", "Use a clearer subject, one source-backed reason for relevance, one approved claim, and a reply path that does not force a meeting too early.", &["PMM"], &["README.md"]),
    ...
    vec![
        (
            ...
            json!({
                ...
                "expect_entry_titles_contains": ["LinkedIn opener"],
                "expect_entry_titles_excludes": ["Email follow-up", "Call prep"]
            }),
        ),
        (
        ...
        (
            ...
            json!({
                ...
                "expect_entry_titles_excludes": ["LinkedIn opener", "Call prep"]
            }),
        ),
        (
            ...
            json!({
                ...
                "expect_entry_titles_excludes": ["LinkedIn opener", "Email follow-up"]
            }),
        ),
        (
