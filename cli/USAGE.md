...
Use `brief` for production GTM prospect handoff. Add `--out <path>` when the brief should be saved; otherwise the artifact is stdout-only. Add `--readable` without global `--json` when a human needs a Markdown review layer over the same prospect/context data; keep `mdp --json brief --context` as the machine source of truth. Do not treat the prospect-readable command as the proposal artifact shape; proposal review artifacts should be profile-aware and should not become blank-page proposal generation, proposal management software, legal/procurement approval, or automated submission. Use `copy` only for local demos. Source inventory lives in `.mdp/sources.yaml`, reusable extraction prompts live in `.mdp/prompts/*.yaml`, CTA guidance lives in `cards/ctas.yaml`, channel rules live in `cards/channel-policies.yaml`, approved claims live in `cards/claims.yaml`, global style and structure rules live in `cards/output-rules.yaml`, and durable unknowns live in `cards/gaps.yaml`. Entries can use `avoid` for blocked literals, `exact_paragraphs` for fixed paragraph counts, and `constraints` for deterministic output limits such as word count, subject word count, subject avoid literals, max questions, and forbidden links, attachments, images, HTML, or tracking.
