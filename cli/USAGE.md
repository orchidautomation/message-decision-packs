...
Portfolio terms do not add primitives. A GTM profile may declare `profile.context_dimensions` such as `product`, `capability`, `solution`, or `segment`, plus generic `context_dimension_dependencies`. Card entries use `scope` to narrow where their existing primitive decision applies. Matching is OR within an entry dimension and AND across dimensions; unscoped entries are global. V1 accepts bounded multi-valued runtime selectors (up to 32 dimensions and 16 values per dimension), with duplicate selectors normalized and deduplicated. Jobs may additionally declare `selector_contract: mdp.job-selectors.v1`; structured routes use closed profile values and emit candidate/rejection inventory without hydrating entry bodies, while jobs without that declaration retain legacy compatibility routing.
