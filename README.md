...
For audit-grade proposal normalization, the runner or host must make a fresh/stateless model call and pass only prompt-declared inputs. `scripts/mdp-proposal-runner.mjs` is the host-neutral local command surface that stages sources, preserves or creates source-audit inputs, calls the native runner, validates prompt output, creates the required receipt, and runs review-support probes. It exposes MCP-shaped local steps for future wrapping, but it is not a hosted MCP server. `mdp run-receipt` records the host-owned boundary plus local artifact hashes for the source audit, prompt output, validation result, runner audit, and downstream files, and blocks if the validation-result hashes do not match the supplied prompt-output/source-audit artifacts or if the runner-audit prompt-output hash does not match the supplied prompt output. Same-conversation, dry-run, or mock normalization without a valid required runner-audit receipt is advisory or blocked even when the JSON validates.
