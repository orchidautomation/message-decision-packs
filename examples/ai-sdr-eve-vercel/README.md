...
`dryRun: true` is the only path that uses the public-safe fixture. Omit `dryRun` to use live Exa when `EXA_API_KEY` is configured; protected live/Cron runs without Exa fail closed with `qualified: 0` and do not append fixture rows. Live Exa runs now do two passes: account trigger discovery, then public person/role resolution. The person lookup is driven by `.mdp/source-strategy.json` via `exa-person-role-resolution.query_template`, so the operator-authored MDP pack controls where Eve looks for person-level resolution.
