...
import type { LedgerRow, SourceStrategyTrace } from "../src/schemas/ledger.ts";
import { loadSourceStrategy, selectScoutQuery, toSourceStrategyTrace } from "../src/scout/source-strategy.ts";

export type ScoutWorkflowInput = {
  packId?: string;
  scheduleId?: string;
  query?: string;
  sourceStrategyPath?: string;
  fixturePath?: string;
  outputDir?: string;
  dryRun?: boolean;
  persist?: boolean;
...
type DiscoveryResult = {
  discovered: CandidateWithEvidence[];
...
export async function scoutCycleWorkflow(input: ScoutWorkflowInput = {}): Promise<ScoutWorkflowResult> {
  ...
  const discovery = await discoverCandidatesStep({ input, config });
  const rows = await mdpScoreRowsStep({ input, config, discovery });
...
async function discoverCandidatesStep(args: { input: ScoutWorkflowInput; config: ScoutConfig }): Promise<DiscoveryResult> {
  "use step";
  ...
  const sourceStrategy = await loadSourceStrategy(args.input.sourceStrategyPath ?? process.env.SCOUT_SOURCE_STRATEGY_PATH);
  const selectedQuery = selectScoutQuery(sourceStrategy, args.input.query);
  ...
  return { discovered, sourceStrategy: toSourceStrategyTrace(selectedQuery), query: selectedQuery.query };
}

async function mdpScoreRowsStep(args: { input: ScoutWorkflowInput; config: ScoutConfig & { runId?: string }; discovery: DiscoveryResult }): Promise<LedgerRow[]> {
  "use step";
  ...
  for (const item of args.discovery.discovered.slice(0, args.config.maxCandidates)) {
    ...
    rows.push({
      ...
      source_strategy: args.discovery.sourceStrategy,
