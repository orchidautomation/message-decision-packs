...
This check intentionally validates the fixture, required file layout, and dry-run contract without requiring `npm install` or live credentials. Install dependencies before running the Next/Vercel app itself.
...
Then test the cron route locally:
...
- `native`: trusted application code runs a packaged Linux `mdp` binary against scratch files under `/tmp`.
- `sandbox`: a Vercel Sandbox microVM receives the pack/prospect files, runs `mdp`, and returns artifacts.
...
- Live Exa/Firecrawl/Apify calls are adapter stubs until credentials and rate-limit policy are added.
