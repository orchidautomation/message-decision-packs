...
- KTD5. **Treat credentials and provider response bodies as private runtime material.** Every successful provider result binds the SHA-256 of the exact raw response bytes, while retained public authority contains only that hash, safe IDs, observed model metadata, timings, and sanitized failure codes. It never includes the bearer token or unrestricted response body.
