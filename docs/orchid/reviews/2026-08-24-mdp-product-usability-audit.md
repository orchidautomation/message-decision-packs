...
1. **Failure-safe authorship:** multi-file pack creation, update, and supported migration must be previewable, validate as a complete candidate, preserve unrelated changes, detect concurrent edits, and avoid a mixed partial pack after handled failure.
2. **Explicit compatibility:** the product must distinguish compatible, deprecated-but-readable, migration-available, unsupported, and retargeting states without silently upgrading authority.
3. **One-command installation:** one supported command installs or upgrades the exact CLI plus one host-native agent plugin containing the MDP skills and canonical local MCP registration, then verifies that all three surfaces are compatible and discoverable.
4. **Authoring and usage are separate journeys:** authoring skills may create or change reviewed pack authority; usage skills consume a selected pack and input to produce a decision or bounded context and must not silently mutate the pack.
5. **Local/cloud contract parity:** every public CLI command has an explicit portability disposition, and every cloud-eligible operation uses the same versioned request/result, decision, reason-code, hash, terminal-state, and receipt semantics as the local authority.

These requirements describe user-visible outcomes. They do not require Git, Compound Engineering, a global artifact registry, a hosted API in the local MVP, or a one-to-one HTTP endpoint for every CLI command.
...
**Outcome:** A new user or agent can run one installer, choose the obvious authoring or usage journey, initialize an MVP pack, understand its readiness, demonstrate one governed job, and recover from ordinary failures without learning the internal assurance architecture first. Core authority, offline defaults, fail-closed behavior, and future local/cloud contract parity remain intact.
**Suggested success measures:** one command produces a compatible CLI plus native host plugin, five skills, and discoverable canonical MCP; zero broken commands in generated starter guidance; median time from install to understandable readiness result under ten minutes and from init under five minutes; every machine-readable error remains parseable; observed skill evals show correct author/use routing, orientation, and handoff; credentialed MCP cannot read outside startup-approved roots; every public command has a tested portability disposition before generalized Cloud adoption.
...
### Issue 25 — Install the CLI, host-native plugin, skills, and canonical MCP in one command

**Priority / size:** P0 / L
**Problem:** The documented `--agents` installer already installs the CLI and Pluxx-generated host bundles, and release smoke proves the MCP scripts are present, but MDP's canonical Pluxx source does not declare an MCP registration and the generated Codex manifest does not expose `mcpServers`. A user can therefore have the binary and skills installed without the canonical MCP being registered or discoverable.
**Scope:** Keep one canonical `pluxx.config.ts` authoring source; declare the profile-neutral local stdio MCP there; use the current native agent-plugin formats generated by Pluxx; make one installer command install or upgrade the CLI, selected/detected host plugin, skills, scripts, hooks where supported, and MCP registration; verify the installed result. Consume Pluxx's trusted legacy-install adoption work rather than reimplementing host ownership and upgrade logic in MDP.
**Non-goals:** Maintaining hand-authored parallel host bundles, installing provider credentials, enabling real model calls, starting an HTTP daemon, silently modifying unsupported hosts, or making the compatibility proposal MCP first-class.
**Acceptance criteria:**

- One documented command is the default full installation path; CLI-only installation remains an explicit advanced/minimal option.
- The full installer installs exactly one verified CLI version and the matching generated plugin bundle for each selected/detected supported host.
- The generated plugin uses the host's current native agent-plugin format and includes the five authored skills plus the canonical profile-neutral local stdio MCP declaration.
- After host restart/reload where required, the MDP skills and `mdp_run_tools`, `mdp_prepare_run`, `mdp_run`, and `mdp_verify_run` are discoverable without manual path editing.
- Installation is idempotent and supports a safe compatible upgrade from trusted legacy generated installs, consuming the Pluxx `PLUXX-333` adoption behavior or its released successor.
- A partial failure rolls back or leaves the prior verified installation active; it never leaves a new CLI paired with an incompatible partial plugin.
- Installed verification checks CLI version, plugin version, skill inventory, referenced bundle files, MCP configuration, MCP tool discovery, and host activation status where observable.
- Installer output gives one concise success state and exact host-reload or repair action when needed.
- No secrets or provider-call enablement are written by installation.

**Implementation evidence:** `docs/getting-started.md:11-35` already documents one `--agents` command; `pluxx.config.ts` is the canonical cross-host source but currently has no `mcp` declaration; release smoke confirms both MCP server scripts are bundled.
**Validation:** fake-home install/upgrade/rollback matrix, generated native-manifest validation, CLI/plugin/skill version parity, MCP tool-discovery smoke, and supported-host installed-runtime checks where the host can be exercised.
**Dependencies:** Pluxx's new agent-plugin generation and trusted legacy-adoption contract, including `PLUXX-333`; Issues 9 and 19 own portability and version-skew behavior; Issues 11-13 own canonical MCP safety and protocol quality.

### Issue 26 — Make pack authorship and pack usage two obvious skill journeys

**Priority / size:** P0 / M
**Problem:** The current five skills already attempt to separate building/reviewing a pack from using it for GTM or proposal decisions, but the coordinator is broad and the dense shared mechanics make ownership feel ambiguous.
**Scope:** Define two user-facing lanes over the same pack contracts: **Author and maintain** creates, updates, validates, and reviews durable pack authority; **Use and decide** selects a released/validated pack and input, runs the appropriate readiness/fit/brief/review path, and returns a decision/bundle without silently editing the pack. Keep specialist skills but make their routing and communication reflect these lanes.
**Non-goals:** Splitting MDP into separate products, duplicating CLI semantics, allowing usage to auto-approve evidence, or preventing an explicitly requested follow-up from moving from usage findings into a new authoring change set.
**Acceptance criteria:**

- The coordinator can explain and route to the two lanes without exposing the internal five-skill topology unless useful.
- Builder owns pack creation and explicit edits; pack review owns read-only diagnosis by default.
- GTM brief and proposal review own pack usage and do not change durable pack authority during a usage run.
- A usage-discovered gap is returned as a bounded finding or proposed authoring handoff, never silently repaired in the same authority state.
- Authoring opens with the authority to be created/changed and closes with validated file changes; usage opens with the selected pack/job/input and closes with the decision, bundle, gaps, and next allowed action.
- Trigger and output evals include author-versus-use near misses, mixed requests, explicit transitions, and refusal of silent cross-lane mutation.
- Both lanes remain usable after the one-command plugin installation from Issue 25.

**Validation:** observed trigger trials, cross-lane mutation tests, authoring and usage golden journeys, and fresh-user comprehension review.
**Dependencies:** Issues 7 and 8 own communication and progressive disclosure; Issues 22 and 23 own bundle handoff and safe authored changes.

### Issue 27 — Define transport-neutral CLI contracts for future Cloudflare adapters

**Priority / size:** P0 / L
**Problem:** The local CLI is the product authority today, while the accepted architecture allows a later Cloudflare-hosted adapter. Without an explicit command/contract map, local filesystem flags and CLI-only presentation choices could harden into semantics that Cloud must duplicate or reinterpret.
**Scope:** Inventory every public CLI command and classify it as local-only operator/filesystem behavior, portable deterministic/core operation, runtime/driver operation, verification operation, or intentionally not cloud-eligible. For each portable operation, define a versioned transport-neutral request/result contract beneath the CLI adapter. Preserve one shared kernel and golden fixture set so a future Worker plus Cloudflare Container adapter can accept bounded inline/content-addressed inputs rather than caller-selected filesystem paths and produce identical authority semantics.
**Non-goals:** Authorizing generalized MDP Cloud, adding an HTTP endpoint for every command, moving filesystem installation/doctor behavior to Cloud, selecting Cloudflare storage products prematurely, or letting Worker/Container ingress become decision authority.
**Acceptance criteria:**

- A committed matrix gives every public command one explicit local/cloud disposition and rationale.
- Cloud-eligible operations have versioned requests/results independent of Clap spelling, terminal formatting, and local absolute paths.
- The CLI adapts local paths into frozen bytes/content-addressed references before invoking the shared core; a cloud adapter accepts bounded inline artifacts or tenant-scoped opaque object references and cannot accept arbitrary server paths.
- Local CLI, MCP, and future Cloud adapters share decision states, reason codes, canonicalization, hashes, no-draft behavior, assurance vocabulary, receipt schemas, and verification rules.
- Cloud-specific auth, tenancy, rate limits, durable replay, storage, queues, observability, and deployment controls remain adapter/host concerns and cannot alter decision policy.
- The existing Cloudflare shape—thin Worker ingress in front of a pinned container/runtime—can consume the contracts without copying Rust authority or adding edge decision logic.
- Golden local/cloud conformance fixtures prove identical deterministic results and explicitly classify allowed host-observation differences.
- Capabilities/version negotiation lets a client determine which operations and contract versions a local or future cloud adapter supports.
- Generalized hosted work remains blocked by the accepted MDP Cloud adoption gate until its security, tenancy, privacy, reliability, cost, and pilot conditions pass.

**Implementation evidence:** `docs/orchid/decisions/2026-08-03-unified-clean-context-runtime.md:127-190` requires one runtime with thin adapters; `docs/orchid/decisions/2026-08-03-mdp-cloud-clean-run-adoption-gate.md` forbids Cloud-specific authority; the sibling Cloudflare design already uses thin Worker ingress before a pinned container and prohibits request-selected filesystem paths.
**Validation:** command disposition contract test, request/result schema fixtures, local versus container differential suite, no-path cloud request tests, hash/reason/terminal-state parity, and adoption-gate review before any generalized hosted exposure.
**Dependencies:** Issue 3 supplies exact capabilities; Issues 5 and 6 supply stable readiness/diagnostic projections; Issues 11-13 define MCP adapter safety; existing MDP-179 through MDP-187 architecture authority remains controlling.
...
| --- | --- | --- |
| 1 — Trust and contract foundation | 1, 2, 3, 11, 22, 27 | Correctness, security, seamless bundle handoff, and local/cloud contract boundaries should precede polish claims. |
| 2 — Golden path | 4, 5, 6, 7, 17, 18, 26 | Create one understandable readiness result and obvious author/use journeys over self-contained bundles. |
| 3 — Simplify, install, and harden | 8, 9, 12, 13, 14, 15, 19, 23, 24, 25 | Make plugins, adapters, installation, authoring, upgrades, temporary state, and recovery predictable. |
...
- Issue 25 should consume released Pluxx native-plugin and legacy-adoption behavior rather than growing host-specific installer ownership in MDP.
- Issue 27 should review new or renamed commands before their local filesystem interface becomes mistaken for portable cloud semantics.
...
| Full install does not guarantee canonical MCP registration/discovery | Issue 25 |
| Authorship and usage are attempted but not presented as two obvious journeys | Issue 26 |
| Public CLI lacks an explicit local/cloud portability disposition matrix | Issue 27 |
