...
- `decision: blocked` when required artifacts are missing, malformed, or failed validation.
...
| --- | --- | --- |
| Pluxx/plugin | Ship skills, hooks, templates, assets, and host-specific bundles | Model invocation context isolation |
| Host runner / future MCP | Fresh stateless normalization call, declared-input payload, local artifact staging, run receipt inputs | Pack fit/routing/proof logic |
| MDP CLI | Validate pack/prompt/proof artifacts, compute hashes, gate receipt status, run deterministic fit/route/checks | Semantic truth beyond supplied artifacts or host context isolation |
