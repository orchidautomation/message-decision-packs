...
When a pack declares `value_contracts` or `attribute_definitions`, prompt output must emit exactly those blessed values. `mdp validate-prompt-output` rejects non-pack personas, non-enum segments/source kinds, invalid date/date-time values, type mismatches, and undeclared attributes when `allow_undeclared_attributes: false`. For prospect/opportunity normalization outputs, `normalization_trace.fit_readiness.ready_for_mdp_fit` must be a boolean and must not be `true` when manifest `lead_input_requirements.required_fields`, `required_signal_fields`, or `required_attributes` are missing from `normalized_prospect`. If source data contains an out-of-contract value or required readiness input is absent, preserve it in `gaps` or `normalization_trace`, set readiness false, and ask for reviewed input or a manifest update; do not silently rename values or claim fit readiness.
