...
- `validate-prompt-output`: validate model-produced normalization or governed-artifact output against the exact selected prompt; pass `--source-audit` for proposal PDF/doc extraction ledgers when raw-field/snippet citations must resolve. Generated prose still requires `check-claims` or `verify-output`.
