...
- `scripts/mdp-run-mcp-server.mjs` (or `${PLUGIN_ROOT}/scripts/mdp-run-mcp-server.mjs` in installed bundles): the only default MCP path. Use `mdp_run_tools` → `mdp_prepare_run` → `mdp_run` → `mdp_verify_run`; the artifacts are the boundary inventory, `mdp.run-request.v1`, run bundle/receipt, and `mdp.run-verification.v1`. MCP adds no authority. The proposal MCP remains compatibility-only for existing v0 consumers.
