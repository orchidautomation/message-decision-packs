...
Use this skill only when the surface is legacy/generic or `profile.id` is `gtm` and `mdp-icp-builder` is allowed. If the surface blocks `mdp-icp-builder`, stop and reroute to the allowed/recommended skill named in the surface. Proposal and Recruiting packs should use their profile skills, not GTM ICP cards.
