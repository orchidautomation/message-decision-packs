...
Own the planning and mutation phases of **Author and maintain**. Plan or build evidence-grounded
...
3. For `source-plan`, return a bounded, non-mutating source inventory and
   authority plan with evidence needs, exclusions, owners, gaps, and the next
   permitted action. Do not invent file changes or continue into extraction.
4. For every mutating mode, work in a complete candidate outside the live pack. Use `author preview`
   and then `author apply`; never make an unsealed multi-file live edit.
5. Refresh/check generated README regions and validate the candidate.
6. Run exact-job `skills`, `requirements`, strict validation, gaps, and eval.
...
classes, durable artifacts, validation/readiness state, and remaining gaps.
For `source-plan`, name the plan artifact or in-response plan, record that no
files were changed and preview/apply was not run, and give the next permitted
action. For mutating modes, report changed files and preview/apply status. Do
not return a usage decision from the authoring result.
